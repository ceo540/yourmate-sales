...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: { name: form.name, department: form.department },
      },
    })
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">소속 사업부</label>
              <select
                value={form.department}
                onChange={(e) => setForm({ ...form, department: e.target.value as Department })}
                className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent bg-white"
              >
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            이미 계정이 있으신가요?{' '}
            <Link href="/login" className="font-semibold hover:underline" style={{ color: '#121212' }}>
              로그인
            </Link>
          </p>
        </div>
