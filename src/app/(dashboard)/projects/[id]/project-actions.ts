...
const SHORT_SUMMARY_PRESET_GUIDE: Record<SummaryPreset, string> = {
  short: `[정리 강도: 짧게]
- **한줄**과 **현황**만 작성. 다음·리스크 섹션 *생략*.
- 현황은 1문장으로 압축.`,
  standard: `[정리 강도: 표준]
- 4섹션 모두 작성 (한줄·현황·다음·리스크).
- 각 섹션 1~2문장.`,
  action: `[정리 강도: 액션 중심 — 실행형 우선]
- 4섹션 모두 작성. **다음** 섹션을 *가장 구체적으로*.
- 다음 섹션 각 항목은 **반드시 [담당자] · [마감시각/날짜] · [다음행동(동사로 시작)]** 3요소 명시.
- 예: "[김수아] · [5/15] · [현수막 시안 컨펌 받기]" — 빈 자리는 "(데이터 없음)" 명시 후 항목 자체는 유지.
- 한줄·현황은 짧게, 다음은 상세히.`,
  deep: `[정리 강도: 깊게]
- 4섹션 모두 + 각 섹션 끝에 \`(근거: log·task·계약 출처)\` 표시.
- 다음 섹션은 [담당자]·[마감]·[다음행동] 3요소 + 출처.
- 예: "[김주임] · [5/15] · [명찰 발주] (근거: 5/2 통화 — outcome: 컨펌 요청)".
...
# 🔴 절대 규칙 (위반 시 출력 무효)
1. **데이터에 없는 내용 추측 금지**. 정보 없으면 "(데이터 없음)" 명시.
2. **모호어 금지**: "조만간" · "추후" · "빠르게" · "필요 시" · "검토" · "예정" — 발견 시 구체 날짜·행동으로 재작성.
3. **다음 섹션 = 실행형 강제**: 각 항목 **[담당자] · [마감시각] · [다음행동]** 3요소 명시. 동사로 시작.
4. 마크다운 코드블록 없이.
...
**다음**
실행 항목 1~3개. 각 항목은 \`[담당자] · [마감] · [다음행동]\` 3요소.

**리스크**
지연·미결·외부 의존 등 *지금 손 안 쓰면 사고 날 항목*. 1~3개. 없으면 줄 자체 생략.
...
# 🔴 절대 규칙 (위반 시 출력 무효)
1. **실행형 우선**: "🔴 즉시 처리 필요" 섹션 각 항목은 \`[담당자] · [마감] · [다음행동(동사 시작)]\` 3요소 명시.
2. **모호어 금지**: "조만간" · "추후" · "빠르게" · "필요 시" · "검토" · "예정" — 구체 날짜·행동으로 재작성.
3. **데이터 없는 추측 금지**: 정보 없으면 "(데이터 없음)" 명시. 일반론·관행 X.
4. 핵심 숫자·날짜·이름은 \`**굵게**\`.
...
**🔴 즉시 처리 필요**
- 마감 임박·미결·고객 답변 대기 등
- *각 항목 형식 강제*: \`[담당자] · [마감] · [다음행동]\` — 빈 자리는 "(데이터 없음)" 명시 후 항목 유지
...
# 🔴 절대 규칙 (실행형 우선)
1. title은 **동사로 시작** + 구체 산출물·대상 명시. 모호어 금지("검토"·"확인"·"준비"·"조만간 처리" 등).
2. due_date는 가능한 한 **구체 날짜**(YYYY-MM-DD). 데이터에 단서 있으면(예: 행사일·계약 단계) 역산.
3. 정말 추정 불가만 due_date=null. 그래도 priority는 *반드시* 명시.
4. 기존 업무와 중복 시 *제안 자체 제외* (이름 살짝 다르게 포장 X).
...
JSON 배열로 반환 (3~6개):
[{"title": "동사 시작 + 구체 산출물", "priority": "긴급|높음|보통|낮음", "due_date": "YYYY-MM-DD 또는 null"}]

좋은 예: \`{"title": "현수막 시안 김수아에게 전달 받기", "priority": "높음", "due_date": "2026-05-08"}\`
나쁜 예: \`{"title": "현수막 검토", "priority": "보통", "due_date": null}\` — 동사 모호·날짜 비움
...
- 각 항목에 *누구·무엇·언제* 명시.`,
  action: `[정리 강도: 액션 중심 — 실행형 우선]
- **🔥 빠르게 해결** 섹션을 *가장 먼저, 가장 구체적으로*. 각 항목은 명령형으로 **[담당자] · [마감] · [다음행동]** 3요소 강제.
- 예: "[김수아] · [5/15] · [현수막 시안 컨펌 받기]" — 빈 값은 "(데이터 없음)" 명시 후 유지.
- ❓·📌·⚠️ 섹션도 가능한 한 같은 3요소 형식 유지. 모르면 (데이터 없음).
- 데이터에 명시된 *다음 행동*만. 추측 금지.`,
  deep: `[정리 강도: 깊게]
- 4섹션 모두 + 각 항목 끝에 \`(근거: log YYYY-MM-DD / task ID / payment label 등)\` 출처 표시.
- 모든 섹션 항목에 [담당자]·[마감]·[다음행동] 3요소 + 출처.
- 예: "[김주임] · [5/2] · [명찰 시안 컨펌 받기] (근거: 4/29 통화 — outcome: 컨펌 요청)".
- 추측 금지. 데이터에 없는 출처는 표시 X.`,
}
...
# 🔴 절대 규칙 (위반 시 출력 무효)
1. **데이터에 없는 내용 추측·창작 금지**. 정보가 없으면 "(데이터 없음)" 명시.
2. **각 항목마다 [담당자] · [마감시각] · [다음행동] 3요소 강제** (특히 action·deep 강도). 빈 자리는 "(데이터 없음)" 표기 후 항목 자체는 유지.
3. **모호어 금지**: "조만간" · "추후" · "빠르게" · "필요 시" · "검토" · "예정" — 발견 즉시 구체 날짜·행동으로 재작성.
4. *${DISCUSSION_LABEL[target]}* 범위 외(${target === 'client' ? '내부·외주사' : target === 'internal' ? '클라이언트·외주사' : '클라이언트·내부'}) 항목은 *완전 생략*.
5. 마크다운, 코드블록 없이. 섹션 헤더는 굵게(**...**).

# 분류 기준 (반드시 이 정의대로 — 라벨 정확히)
- **🔥 빠르게 해결**: 오늘(${todayIso}) ~ 내일까지 처리 가능, 의존성 없음. 즉시 액션.
- **❓ 확인 필요**: 우리가 답을 줄 수 없음 — 클라이언트/내부/외주사에 *물어봐야* 결론 남.
- **📌 결정 사항**: 의사결정이 필요한 항목 (방향·규모·예산 결정). 또는 우리가 발송 후 상대 답변 대기.
- **⚠️ 리스크**: 마감 임박·지연·외부 의존·전제 깨짐 등 *지금 손 안 쓰면 사고날* 항목.

# 자동 추출 신호 (놓치지 마)
- pending_task.due_date < 오늘 → 반드시 ⚠️ 리스크
- pending_task.due_date 7일 이내 → ⚠️ 또는 🔥
- log.outcome에 "보냄/전달함/요청함" 표현 → 📌 결정 사항 (답변 대기 중)
- log.outcome에 "협의 필요/결정 필요/확인 부탁" → ❓ 확인 필요 필수
- task.assignee_id IS NULL → ❓ "담당 미배정 — 누구에게 배정?"
...
# 출력 형식 (마크다운, 코드블록 X — 라벨 정확히 일치)

**🔥 빠르게 해결**
- 각 항목: \`[담당자] · [마감(날짜/시각)] · [다음행동(동사 시작)]\`

**❓ 확인 필요**
- 각 항목: 누구에게 / 무엇을 / 언제까지 답변 받아야 하는지

**📌 결정 사항**
- 각 항목: 무엇을 / 누가 결정해야 하는지 / 언제까지

**⚠️ 리스크**
- 각 항목: 무엇이 / 마감 또는 D-N / 담당자 / 손 안 쓰면 무엇이 깨지는지
