...
const CUSTOMERS = [
  { id: 1, name: '이화여자대학교', type: '학교', region: '서울', contacts: 2, deals: 5 },
  { id: 2, name: '경기도교육청 특수교육원', type: '공공기관', region: '경기', contacts: 3, deals: 3 },
  { id: 3, name: '광교청소년수련관', type: '공공기관', region: '경기', contacts: 1, deals: 2 },
  { id: 4, name: '강남구청', type: '공공기관', region: '서울', contacts: 2, deals: 4 },
  { id: 5, name: '용인교육지원청', type: '공공기관', region: '경기', contacts: 2, deals: 6 },
  { id: 6, name: '인천해양고등학교', type: '학교', region: '인천', contacts: 1, deals: 3 },
]
...
const STATUS_COLOR: Record<string, string> = {
  '리드': '#3B82F6', '계약': '#8B5CF6', '진행중': '#F59E0B', '완료': '#6B7280',
}
const STATUS_BG: Record<string, string> = {
  '리드': '#EFF6FF', '계약': '#F5F3FF', '진행중': '#FFFBEB', '완료': '#F9FAFB',
}
...
export default function RedesignDemo() {
  ...
  const filtered = PROJECTS.filter(p =>
    (filterService === '전체' || p.service === filterService) &&
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#F4F5F7', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', fontSize: 14, color: DARK }}>
      ...
      <div style={{ width: sidebarOpen ? 220 : 60, background: DARK, display: 'flex', flexDirection: 'column', transition: 'width 0.2s', flexShrink: 0 }}>
        ...
        <button onClick={() => setSidebarOpen(o => !o)}
          style={{ padding: '8px', background: 'transparent', border: 'none', color: '#6B7280', cursor: 'pointer', fontSize: 16 }}>
          {sidebarOpen ? '←' : '→'}
        </button>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                {PROJECTS.filter(p => p.dday !== null && p.dday <= 14).sort((a, b) => (a.dday ?? 99) - (b.dday ?? 99)).map(p => (
                  <div key={p.id} onClick={() => openProject(p)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #F3F4F6', cursor: 'pointer' }}>
                    ...
                    <Badge label={p.service} color={SERVICE_COLOR[p.service] || '#6B7280'} bg={`${SERVICE_COLOR[p.service]}18` || '#F3F4F6'} />
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ marginTop: 12, padding: '10px 14px', background: '#F9FAFB', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#6B7280' }}>구글 캘린더 연동됨</span>
                  <span style={{ marginLeft: 'auto', fontSize: 11, color: '#059669', fontWeight: 600 }}>동기화 중</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: 28 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                ...
                <p style={{ color: '#6B7280', margin: '4px 0 0', fontSize: 13 }}>전체 {PROJECTS.length}건 · 진행중 {PROJECTS.filter(p => p.status === '진행중').length}건</p>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {['전체', 'SOS', '교육프로그램', '납품설치', '교구대여', '콘텐츠제작', '행사운영', '002ENT'].map(s => (
                  <button key={s} onClick={() => setFilterService(s)}
                    style={{ padding: '7px 12px', borderRadius: 20, border: `1px solid ${filterService === s ? (SERVICE_COLOR[s] || YELLOW) : '#E5E7EB'}`, background: filterService === s ? (SERVICE_COLOR[s] || YELLOW) : '#fff', color: filterService === s ? '#fff' : DARK, fontSize: 11, cursor: 'pointer', fontWeight: 600 }}>
                    {s}
                  </button>
                ))}
              </div>
        ...
        {page === 'project-detail' && selectedProject && (
          <div style={{ padding: 28 }}>
            {/* Back */}
            <button onClick={() => setPage('projects')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6B7280', fontSize: 13, padding: 0, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
              ← 프로젝트 목록
            </button>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    ...
                    <Badge label={selectedProject.service} color={SERVICE_COLOR[selectedProject.service] || '#6B7280'} bg={`${SERVICE_COLOR[selectedProject.service]}18` || '#F3F4F6'} />
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button style={{ padding: '8px 14px', border: '1px solid #E5E7EB', borderRadius: 8, background: '#fff', fontSize: 12, cursor: 'pointer' }}>📅 구글캘린더</button>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
              <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {selectedProject.service === 'SOS' && (
                      <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderLeft: `4px solid ${SERVICE_COLOR['SOS']}` }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                          {[['공연일', '2026-05-03 (토)'], ['공연 장소', '이화여대 대강당'], ['대상', '전교생 800명'], ['공연 시간', '90분 (14:00~15:30)'], ['아티스트', '3팀 섭외 완료'], ['특이사항', '장기자랑 10분 추가']].map(([k, v]) => (
                            <div key={k}>
                              <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 2 }}>{k}</div>
                              <div style={{ fontSize: 13, fontWeight: 600 }}>{v}</div>
                            </div>
                    ...
                    {selectedProject.service === '교구대여' && (
                      <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderLeft: `4px solid ${SERVICE_COLOR['교구대여']}` }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                          {[['대여 기간', '2026-04-15 ~ 2026-04-23'], ['반납 방법', '택배 반납 (착불)'], ['보증금', '없음'], ['배송 주소', '서울시 강남구...'], ['품목', '아이패드 20대, 케이스 20개'], ['결제 상태', '결제 완료']].map(([k, v]) => (
                            <div key={k}>
                              <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 2 }}>{k}</div>
                              <div style={{ fontSize: 13, fontWeight: 600 }}>{v}</div>
                            </div>
                        ...
                        <div style={{ marginTop: 16, background: '#FFF7ED', borderRadius: 8, padding: 12 }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: '#D97706', marginBottom: 6 }}>⚠️ 반납 D-2 — 수거 준비 필요</div>
                          <div style={{ fontSize: 12, color: '#92400E' }}>택배 반납 예정 · 착불 · 반납 후 검수 필요</div>
                        </div>
                      </div>
                    )}
                    ...
                    {!['SOS','교구대여','교육프로그램'].includes(selectedProject.service) && (
                      <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderLeft: `4px solid ${SERVICE_COLOR[selectedProject.service] || '#E5E7EB'}` }}>
                        <h3 style={{ margin: '0 0 14px', fontSize: 14, fontWeight: 700, color: SERVICE_COLOR[selectedProject.service] || '#6B7280' }}>🔧 {selectedProject.service} 정보</h3>
                ...
                {projectTab === 'comms' && (
                  <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                    <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>소통 내역</h3>
                    {[
                      { date: '2026-04-21', author: '조민현', content: '담당자 통화 완료. 공연 날짜 5월 3일로 확정. 아티스트 3팀 요청함.', channel: '전화' },
                      { date: '2026-04-18', author: '방준영', content: '견적서 발송 완료. 250만원 부가세 포함 기준으로 발송.', channel: '이메일' },
                      { date: '2026-04-15', author: '조민현', content: '최초 문의 접수. 채널톡으로 유입. 5월 공연 희망.', channel: '채널톡' },
                    ].map((c, i) => (
                      <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                        <div style={{ width: 32, height: 32, background: YELLOW, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{c.author[0]}</div>
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 13, color: '#374151', background: '#F9FAFB', padding: '10px 14px', borderRadius: 8 }}>{c.content}</div>
                        </div>
                      </div>
                    ))}
                    <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                      <input placeholder="소통 내용 입력..." style={{ flex: 1, padding: '10px 14px', borderRadius: 8, border: '1px solid #E5E7EB', fontSize: 13, outline: 'none' }} />
                      <button style={{ background: YELLOW, border: 'none', borderRadius: 8, padding: '10px 16px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>저장</button>
                    </div>
                  </div>
                )}
                ...
                {projectTab === 'finance' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                      <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>재무 현황</h3>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                        {[
                          { label: '매출액', value: '250만', color: '#059669' },
                          { label: '원가', value: '110만', color: '#EF4444' },
                          { label: '수익', value: '140만 (56%)', color: '#2563EB' },
                        ].map(f => (
                    ...
                    <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                      <h3 style={{ margin: '0 0 16px', fontSize: 14, fontWeight: 700 }}>수금 일정</h3>
                      {[
                        ...
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid #F3F4F6' }}>
                          ...
                          {s.done && <span style={{ fontSize: 11, color: '#059669', background: '#F0FDF4', padding: '2px 8px', borderRadius: 10, fontWeight: 600 }}>수령완료</span>}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {projectTab === 'claude' && (
                  <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                    ...
                    <div style={{ marginTop: 10, display: 'flex', gap: 6 }}>
                      <button style={{ fontSize: 11, padding: '4px 10px', borderRadius: 20, border: '1px solid #E5E7EB', background: '#fff', cursor: 'pointer', color: '#6B7280' }}>📋 MD 저장</button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ background: '#fff', borderRadius: 12, padding: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                  ...
                  {[
                    ['고유번호', selectedProject.id],
                    ['상태', selectedProject.status],
                    ['서비스', selectedProject.service],
                    ['담당자', selectedProject.assignee],
                    ['단계', selectedProject.stage || '—'],
                  ].map(([k, v]) => (
                ...
                <div style={{ background: '#fff', borderRadius: 12, padding: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                  ...
                  {[
                    { icon: '💬', label: '채널톡 대화', sub: '최근 3건', color: '#2563EB' },
                    { icon: '📧', label: '이메일 스레드', sub: '2개 연결됨', color: '#059669' },
                    { icon: '📅', label: '구글 캘린더', sub: '일정 2개', color: '#EF4444' },
                  ].map((c, i) => (
        ...
        {page === 'customers' && (
          <div style={{ padding: 28 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
              {CUSTOMERS.map(c => (
                <div key={c.id} style={{ background: '#fff', borderRadius: 12, padding: 18, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                    <span style={{ color: '#6B7280' }}>담당자 <strong>{c.contacts}명</strong></span>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
                {Array.from({ length: 35 }, (_, i) => {
                  const day = i - 3
                  const hasEvent = CALENDAR_EVENTS.find(e => {
                    const d = parseInt(e.date.split('-')[2])
                    return d === day && e.date.startsWith('2026-05')
                  })
                  return (
                    <div key={i} style={{ minHeight: 60, padding: 6, borderRadius: 8, background: day === 21 ? `${YELLOW}20` : '#F9FAFB', border: day === 21 ? `1px solid ${YELLOW}` : '1px solid transparent' }}>
                      {day > 0 && day <= 31 && (
                        <>
                          <div style={{ fontSize: 12, fontWeight: day === 21 ? 800 : 400, color: day === 21 ? DARK : '#374151', marginBottom: 4 }}>{day}</div>
                          {hasEvent && (
          ...
          <div style={{ padding: 28 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120 }}>
                {[
                  { month: '1월', val: 45 }, { month: '2월', val: 30 }, { month: '3월', val: 65 },
                  { month: '4월', val: 75 }, { month: '5월', val: 0 }, { month: '6월', val: 0 },
                ].map(m => (
                  <div key={m.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <div style={{ width: '70%', background: m.val ? YELLOW : '#F3F4F6', borderRadius: '4px 4px 0 0', height: m.val ? `${m.val}%` : '4px', transition: 'height 0.3s' }} />
                ...
                <div key={m.name} style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, fontSize: 13 }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontWeight: 800, fontSize: 20, color: DARK }}>{m.tasks}</div>
                      <div style={{ color: '#9CA3AF', fontSize: 11 }}>진행 업무</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontWeight: 800, fontSize: 20, color: DARK }}>{m.projects}</div>
