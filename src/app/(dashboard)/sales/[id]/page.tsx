...
export default async function SalePage({ params, searchParams }: {
  ...
  const { data: sale } = await admin.from('sales').select('*, notes, project_overview, project_id, share_token').eq('id', id).single()
  if (!sale) notFound()
  ...
  if (!isAdmin && !isProjectAssignee && !isContractAssignee) redirect('/sales')
