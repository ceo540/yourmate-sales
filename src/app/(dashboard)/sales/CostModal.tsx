...
const AUTO_INNER = [
  { item: '감가상각비', rate: 0.06 },
  { item: '지급수수료', rate: 0.02 },
  { item: '부가세', rate: 0.10 },
]
...
export default function CostModal({ saleId, saleName, revenue, initialItems, vendors, onClose }: Props) {
  ...
  const autoInsertInner = async () => {
    ...
    const { data } = await supabase.from('sale_costs')
      .upsert(rows, { onConflict: 'sale_id,item', ignoreDuplicates: true })
      .select()
  ...
  const renderSection = (category: Category, sectionItems: CostItem[]) => {
    ...
    const outerVendors = vendors // 외부원가에서만 거래처 선택
      ...
      <div className="mb-5">
        ...
        {templates.length > 0 && (
          <div className="flex gap-1.5 mb-2 flex-wrap">
            {templates.map(t => (
              <button key={t}
                onClick={() => { setAddingIn(category); setForm({ item: t, amount: '', memo: '', vendorId: '' }) }}
        ...
        {/* 거래처 빠른 선택 (외부원가만) */}
        {category === '외부원가' && outerVendors.length > 0 && (
          <div className="flex gap-1.5 mb-2 flex-wrap">
            {outerVendors.map(v => (
              <button key={v.id}
                onClick={() => { setAddingIn(category); setForm({ item: v.name, amount: '', memo: '', vendorId: v.id }) }}
          ...
          {sectionItems.map(item => (
            <div key={item.id} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 group">
              <div className="flex-1 min-w-0">
                ...
                {category === '내부원가' && revenue > 0 && (
                  <span className="text-xs text-gray-300 ml-1.5">{Math.round((item.amount / revenue) * 100)}%</span>
                )}
              </div>
              <span className="text-sm font-medium text-gray-700 flex-shrink-0">{item.amount.toLocaleString()}원</span>
              {/* 외부원가에만 지급 체크 */}
              {category === '외부원가' && (
                <button
        ...
        {addingIn === category ? (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 space-y-2">
            ...
            <div className="flex gap-2">
              <input autoFocus placeholder="항목명" value={form.item}
      ...
      <div className="bg-white rounded-xl w-full max-w-lg shadow-xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleClose} className="px-5 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: '#FFCE00', color: '#121212' }}>
              완료
            </button>
          </div>
        </div>
