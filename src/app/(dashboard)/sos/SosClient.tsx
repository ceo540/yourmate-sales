...
import {
  createConcert, updateConcert, updateConcertStage, updateConcertTasks, deleteConcert,
} from './actions'
...
type ConcertInput = Omit<Concert, 'id' | 'created_at'>
...
// ─── 선생님 검색 ──────────────────────────────────────────────────
const ALL_TEACHERS = Object.entries(MOCK_TEACHERS).flatMap(([, list]) =>
  list.map(t => ({ ...t }))
)
...
function ConcertModal({ concert, onClose, onSave }: {
  ...
  onSave: (data: ConcertInput) => void
}) {
  const [tab, setTab] = useState<'basic' | 'event'>('basic')
  const [form, setForm] = useState({
    year: concert?.year ?? new Date().getFullYear(),
    ...
    tasks_done: concert?.tasks_done ?? 0,
    tasks_total: concert?.tasks_total ?? 11,
    event_info: concert?.event_info ?? {} as EventInfo,
    sale_id: concert?.sale_id,
  })

  const inp  = BASE_INP
  const lbl  = 'block text-xs font-medium text-gray-500 mb-1'
  const set  = (k: string, v: unknown) => setForm(f => ({ ...f, [k]: v }))
  const setI = (k: keyof EventInfo, v: unknown) => setForm(f => ({ ...f, event_info: { ...f.event_info, [k]: v } }))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.event_info.venue_clear ?? ''} onChange={e => setI('venue_clear', e.target.value)} className={inp} />
                </div>
              </div>

              <div>
                <label className={lbl}>담당 선생님 <span className="text-blue-400 text-[10px]">고객 DB</span></label>
                <TeacherCombo
                  school={form.school}
                  name={form.event_info.teacher_name ?? ''}
                  phone={form.event_info.teacher_phone ?? ''}
                  onSelect={(n, p) => { setI('teacher_name', n); setI('teacher_phone', p) }}
                />
              </div>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t">
          ...
          <button
            onClick={() => {
              onSave({ ...form, tasks_done: concert ? concert.tasks_done : STAGE_TASKS[form.stage] })
              onClose()
            }}
...
function StaffAvatars({ names }: { names: string[] }) {
  if (!names || names.length === 0) return <span className="text-xs text-gray-300">미배정</span>
  return (
...
// ─── 공연 카드 ────────────────────────────────────────────────────
function ConcertCard({ concert, isAdmin, onEdit, onDelete, onStageChange, onTaskToggle }: {
  concert: Concert; isAdmin: boolean; onEdit: () => void; onDelete: () => void
      ...
      <div className="px-5 py-4 flex items-center gap-4 cursor-pointer hover:bg-gray-50/60 transition-colors" onClick={() => setExpanded(v => !v)}>
        ...
        <div className="flex-1 min-w-0">
          {concert.mc || (concert.artists?.length > 0) ? (
            <div className="flex flex-wrap gap-1">
              ...
              {(concert.artists?.length ?? 0) > 3 && <span className="text-xs text-gray-400">+{concert.artists.length - 3}</span>}
            </div>
        ...
        <div className="flex-shrink-0" onClick={e => e.stopPropagation()}>
          {isAdmin ? (
            ...
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STAGE_BADGE[concert.stage]}`}>{concert.stage}</span>
          )}
        </div>
        {isAdmin && (
        ...
        <div className="border-t border-gray-100 bg-gray-50/40" onClick={e => e.stopPropagation()}>
          ...
          <div className="px-5 pt-4 pb-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {isAdmin && <button onClick={onEdit} className="text-xs text-yellow-600 hover:underline">편집</button>}
              </div>
            </div>
            {hasInfo ? (
              ...
              <p className="text-xs text-gray-300">
                행사 정보가 없습니다.{' '}
                {isAdmin && <button onClick={onEdit} className="text-yellow-600 hover:underline">추가하기</button>}
              </p>
            )}
          </div>
          ...
          <div className="px-5 py-3 border-t border-gray-100">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-1.5">
              ...
              {!concert.mc && !concert.artists?.length && !concert.staff?.length && (
          ...
          <div className="px-5 py-3 border-t border-gray-100">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {SOS_TASKS.map((task, i) => {
                const isDone = i < concert.tasks_done
                return (
                  <button key={task.key} onClick={() => onTaskToggle(task.key)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-left transition-colors ${
                      isDone ? 'bg-white border border-green-100 text-gray-400' : 'bg-white border border-gray-200 text-gray-700 hover:border-yellow-300'
                    }`}>
                    <div className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center ${isDone ? 'bg-green-400 border-green-400' : 'border-gray-300'}`}>
                      {isDone && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 12 12"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    </div>
                    <span className={`text-xs leading-tight ${isDone ? 'line-through' : ''}`}>{task.label}</span>
                  </button>
                )
              })}
            </div>
...
// ─── 메인 ─────────────────────────────────────────────────────────
export default function SosClient({ concerts: initialConcerts, isAdmin }: { concerts: Concert[], isAdmin: boolean }) {
  ...
  function handleSave(data: ConcertInput) {
    if (modal.concert) {
      setConcerts(cs => cs.map(c => c.id === modal.concert!.id ? { ...c, ...data } : c))
      startTransition(async () => { await updateConcert(modal.concert!.id, data) })
    } else {
      const temp: Concert = { id: `temp-${Date.now()}`, ...data }
      setConcerts(cs => [temp, ...cs])
      startTransition(async () => { await createConcert(data) })
    }
    setModal({ open: false })
  }
  ...
  function handleTaskToggle(concertId: string, taskKey: string) {
    ...
    if (idx === -1) return
    const newDone = idx < concert.tasks_done ? idx : idx + 1
    setConcerts(cs => cs.map(c => c.id !== concertId ? c : { ...c, tasks_done: newDone }))
    startTransition(async () => { await updateConcertTasks(concertId, newDone) })
  }
            ...
            <div key={month}>
              ...
              <div className="space-y-2">
                {byMonth[month].map(concert => (
                  <ConcertCard key={concert.id} concert={concert} isAdmin={isAdmin}
