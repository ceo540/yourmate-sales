      ...
      try {
        ...
        while (true) {
          ...
          for (const block of finalMsg.content) {
            ...
            // P2-4 audit: write 도구만 기록 (read 노이즈 제거)
            const isReadOnly =
            ...
            if (!isReadOnly) {
              ...
              void recordAudit({
                ...
                entity_type: projectId ? 'project' : (saleId ? 'sale' : 'lead'),
                entity_id: projectId ?? saleId ?? leadId ?? null,
                after: { tool: block.name },
                source: 'bbang_project',
                summary: `빵빵이(프로젝트) 도구 호출 — ${block.name}`,
              })
            }
          }
