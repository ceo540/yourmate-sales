...
- **2026-05-01 3차** (라운드 21):
  - §2.1 SOS 정정 (발표회 사업부 X, 002C가 발표회·축제)
  - §2.1.1 두 시각 차이 (기관=1계약 / 내부=사업부 겸업)
  - §2.5 신규: 002C "행사운영" 도피처 + 사업부·서비스 분류 재정리 (방향 A·B·C)
  - §3.2.1 N:M 마이그 영향 분석
  - §5.0.1.a 원가 PDF 자동 기입 재시도 (PoC 있음, 검증 필요)
...
### 2.5 002 Creative "행사운영" = 복합 도피처 ⭐ (사업부·서비스 재정리 신호)

**사용자 답변 (라운드 21):** "002 Creative는 특수하게 *행사운영* 카테고리. 다양·복합 서비스(디자인·인쇄·영상·행사인력·음향·조명) 종합. **복합적인 계약은 다 행사운영으로 넣고 있음.**"

#### 현 상태 = 분류 깨짐 신호

- 서비스 11종 (`SOS`, `교육프로그램`, `납품설치`, ..., `행사운영`)
- 그런데 *"행사운영"* 1종이 *복합 계약 다 흡수* — 다른 서비스 다 안 맞을 때 도피처
- 결과: `service_type='행사운영'` 행이 *실제로 무슨 사업인지* 알 수 없음. 숨은 N:M 관계.

#### 사용자 결론: "사업부·서비스 정리할 타이밍"

**다음 라운드 작업 후보 (우선순위 매트릭스 별도 항목 추가):**

#### 사업부·서비스 분류 재정리 가능한 방향 3가지

**방향 A — 서비스 *복합 표시* 활성**
- service_type 단일 → multi-tag (project_services join)
- 예: "행사운영" 하나 대신 [디자인, 인쇄, 영상, 행사인력] 다중 태그
- 장점: 분류 정확. 영업이익 분석·검색 정확
- 단점: UI 복잡해짐

**방향 B — *서비스 분류 자체 재설계***
- 현 11종 → 2~3 카테고리 + 서브 카테고리
- 예: [공연] [교육] [제작] [운영] [납품] 5개 대분류 + 각 안에 서브
- "행사운영" 같은 도피처 카테고리 자체 재정의
- 장점: 깔끔. 사업부 겸업 자연스럽게 표현
- 단점: 마이그 큼 (모든 sales/leads/projects 재분류)

**방향 C — *현 상태 유지 + 행사운영만 확장***
- "행사운영" 클릭 시 *서브 카테고리 다중 선택* (디자인+인쇄+영상 ✓)
- 다른 서비스는 그대로
- 장점: 마이그 작음
- 단점: 근본 해결 X
...
**N:M sale_projects (§3.2) 마이그 *후* 진행** — 현 시스템 정리 *이후* 서비스 분류 재설계가 자연스러움. 그 사이 사용자가 *행사운영 케이스 누적 데이터*로 어떤 분류가 좋을지 판단.

**우선순위 매트릭스 추가:** "사업부·서비스 분류 재정리" → 후순위 (15~20번대).
...
#### 5.0.1.a 원가 PDF 자동 기입 — 재시도 ⭐ (사용자 라운드 21)

**사용자 답변:** "각 프로젝트 폴더 내 0행정 내 원가 폴더에 넣어놓고 빵빵이가 자동 기입 시도하다가 *실패함*. 자동화 좋겠음."

**현 상태 (메모리 `yourmate-cost-pdf-extract-spec.md` 확인):**
- ✅ PoC 코드 구현 완료 (`33c3919` / `65be221`)
  - `src/lib/cost-pdf-extract.ts` — LLM 호출 + JSON 파싱
  - `src/app/(dashboard)/sales/[id]/cost-actions.ts` — `analyzeCostFolder` / `bulkInsertCosts`
  - `CostPdfImportModal.tsx` — 결과 미리보기 + 일괄 추가
  - SaleHub 원가 탭 + ProjectV2 ContractRow에 [📎 원가 폴더 분석] 버튼
- ⚠️ **운영 PDF 검증 미완** — 사용자 검증 시점에 결과 부족했을 가능성

**재시도 작업 (코드 작업 시 수행):**

1. **모델 검증** — PoC가 gpt-4o 사용. 4-30 세션에서 GPT 라우트 정리됨. 현재 Claude Sonnet 4.6으로 마이그됐는지 코드 확인
2. **운영 PDF 1~3개로 실증** — 사용자 임의 프로젝트 골라 결과 점검
3. **결과 부족하면 LLM 프롬프트 강화:**
   - PDF 종류 인식: 견적서 / 세금계산서 / 거래명세서 / 이체확인증
   - 중복 제거 로직: 같은 거래가 여러 PDF에 → 1건 통합
   - 신뢰도 낮은 행 제외 기준 강화
   - 부가세 처리 명확화 (총액 그대로 vs 별도 분리)
4. **실패 케이스 빵빵이 학습** (§4.6.4 피드백 루프 활용):
   - 사용자가 잘못된 추출 행 [👎] 클릭 → 빵빵이 재학습
   - 누적되면 프롬프트에 *교정 사례* 자동 추가

**우선순위:** N:M 마이그 직후 *3~4시간 검증*. 결과 좋으면 그대로 운영, 부족하면 프롬프트 1~2회 개선.
...
| **1.5** | **원가 PDF 자동 기입 검증·재시도** ⭐ | ⭐⭐ | 3~4시간 | 1순위 직후 | §5.0.1.a |
...
| 19 | **사업부·서비스 분류 재정리** (행사운영 도피처 해소) ⭐ | ⭐⭐ | 1주+ | 후 (N:M 마이그 후) | §2.5 |
| 20 | 외부 노출 (고객·외부인력 포털) | ⭐⭐ | 4주+ | 장기 | §5.11 |
...
- [ ] 모바일 PWA vs 네이티브 (§4.7)
- [ ] **사업부·서비스 분류 재정리 방향 A·B·C 중 결정** (§2.5)
- [ ] **원가 PDF 자동 기입 검증** — 운영 PDF로 실제 결과 확인 (§5.0.1.a)
